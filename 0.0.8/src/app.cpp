...
std::string build_response(const http::Response& res) {
    ...
    switch (static_cast<int>(res.status)) {
        case static_cast<int>(http::Status::OK): response.append(HTTP_OK); break;
        case static_cast<int>(http::Status::NotFound): response.append(HTTP_404); break;
        case static_cast<int>(http::Status::InternalError): response.append(HTTP_500); break;
...
private:
    ...
    void handle_client(std::shared_ptr<RawSocket> client) {
        if (!client || !client->is_valid()) return;
