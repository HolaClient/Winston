...
try {
	...
	function parseJSON(req, res, next) {
		...
		req.body = req.body || {};
		if (!('transfer-encoding' in req.headers || ('content-length' in req.headers && req.headers['content-length'] !== '0'))) return next();
	...
	function server(opts = {}) {
		...
		function static(a, b) {
			if (!fs.existsSync(b)) {
				throw new Error(`Directory ${b} does not exist.`);
			}
			add('GET', a + '/*', async (req, res) => {
				const c = path.join(b, req.path.replace(a, ''));
